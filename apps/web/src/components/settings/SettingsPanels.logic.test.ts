...
describe("project grouping toggle", () => {
  it("enables repository path grouping and disables into separate projects", () => {
    ...
    expect(projectGroupingModeFromToggle(true)).toBe("repository_path");
    expect(projectGroupingModeFromToggle(false)).toBe("separate");
  });

  it("restores repository grouping when the toggle is cycled", () => {
    expect(projectGroupingModeFromToggle(false, "repository")).toBe("separate");
    expect(projectGroupingModeFromToggle(true, "repository")).toBe("repository");
  });
});
